...
int run_cli(int argc, char** argv) {
    ...
    if (cmd == "ask") {
        ...
        size_t cfg_bm25_topk = cfg.get_size("bm25.top_k", 10);
        size_t cfg_max_ranked = cfg.get_size("retrieval.max_ranked_docs", 10);
        size_t cfg_max_evidence = cfg.get_size("retrieval.max_evidence", 15);
        size_t cfg_chunks_per_doc = cfg.get_size("chunk.max_per_doc", 8);
        double cfg_bm25_w = cfg.get_double("retrieval.bm25_weight", 0.7);
        double cfg_ann_w = cfg.get_double("retrieval.ann_weight", 0.3);
        ...
        for (auto& need : needs) {
            // --- Retrieval ---
            auto bm25_results = bm25.search(need.keywords, cfg_bm25_topk);
            ...
            if (use_hybrid && hnsw) {
                ...
                for (uint32_t id : ann_ids) {
                    ...
                    doc_scores[doc] = cfg_bm25_w * bm25_norm + cfg_ann_w * ann_norm;
                }
                    ...
                    for (auto& [doc, sc] : doc_scores) {
                        if (max_bm25 > 0) {
                            ...
                            if (!was_ann) sc = cfg_bm25_w * (sc / max_bm25);
                        }
                    }
                }
            }
            ...
            if (ranked_docs.size() > cfg_max_ranked) ranked_docs.resize(cfg_max_ranked);
