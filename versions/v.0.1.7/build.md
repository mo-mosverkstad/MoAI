# v.0.1.7 — Build & Usage Guide (WSL Ubuntu)
...
Training data: auto-generated from ingested documents — proper entities (capitalized phrases, acronyms) are extracted from documents and combined with templates from `config/vocabularies/language.conf` `[TEMPLATES]` section. No manual labeling needed. With 21 documents, generates ~9,000 high-quality training samples (max 20 entities per document × 22 templates).
